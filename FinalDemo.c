...
//Han 11/24 update comments on FalseBuzzer.c
...
// Variabele to store the number of milleseconds since the board has started
...
int main(void)
{
    ...
    buzzer(); //USER MUST PRESS THE BUTTON AFTER THE BUZZER STOPS!!!!!!!!!!!!!!!
    while (true)
    {
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, 1); //Siren sound
        check();
        currentMillis = HAL_GetTick(); //get time
        readButtonState();
    }
...
void check()
{
    ...
    if (input[0] == note[0] && input[1] == note[1] && input[2] == note[2] && input[3] == note[3] && input[4] == note[4]) //Successful as user enter the correct pattern
    {
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_6, 1); //led lights up
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, 0); //buzzer goes off
        gameWon += 1;
    }
    else if (i == 5)
    { //To ensure that all 5 entries are inputted
        char buff[100];
...
void readButtonState()
{
    ...
    if (currentMillis - previousButtonMillis > intervalButton)
    {
        ...
        if (buttonState == 0 && !buttonStateLongPress && buttonPressDuration >= minButtonLongPressDuration)
        {
            ...
            input[i] = 1;
            i++; //replace 2 with 1 in the particular positon of array(means long press)
            char buff[100];
        ...
        if (buttonState == 1 && buttonStatePrevious == 0)
        {
            ...
            {
                input[i] = 0;//replace 2 with 0 in the particular positon of array(means short press)
                i++;
                char buff[100];
