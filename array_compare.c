//Justin Kim 11/21
//This is a function that integrates led with longpress function & stores input
//Modified readButtonState() and created checker()
#include <stdbool.h> 
#include <stdio.h>   
#include <stdlib.h>
...
if(currentMillis - previousButtonMillis > intervalButton) {
    ...
    if (i > 4) {
     char buff[100];
      // print out value when all the inputs are received
      sprintf(buff, "STOP!");
      SerialPuts(buff);
    }
...
void checker() { //Checking two arrays; not done yet
  InitializePin(GPIOA, GPIO_PIN_7, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, 0);
  for (unsigned int i = 0; i < 5; i++) {
    if (input[i] != note[i]) {
      gameWon = false;
      memset(input, 0, sizeof(input)); // Resetting array
    }
    else if (input[i] == note [i]) {
      gameWon = true;
      char buff[100];
      sprintf(buff, "Game Won!");
      SerialPuts(buff);
      HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, 1); // LED ON
    }
  }
}
